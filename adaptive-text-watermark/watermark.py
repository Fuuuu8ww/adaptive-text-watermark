...
class Watermark():
    ...
    def _next_token_entropy(self, input_text, model, tokenizer, device):
        # model 在这里是 self.measure_model
        # tokenizer 在这里是 self.measure_tokenizer
        
        max_len_for_model_input = getattr(model.config, 'max_position_embeddings', 1024) - 1 
        if max_len_for_model_input <= 0: max_len_for_model_input = 1

        # 使用 self.measure_tokenizer 进行编码以获取长度
        input_ids_full = tokenizer.encode(input_text, return_tensors='pt', add_special_tokens=False)
        original_token_len = input_ids_full.size(1)

        input_ids_to_model = input_ids_full # 默认使用完整输入

        if original_token_len > max_len_for_model_input:
            model_name_for_debug = model.config._name_or_path if hasattr(model, 'config') and hasattr(model.config, '_name_or_path') else "Unknown Model"
            print(f"WARNING (_next_token_entropy for {model_name_for_debug}): "
                f"Input token length ({original_token_len}) exceeds model max ({max_len_for_model_input}). "
                f"Truncating to {max_len_for_model_input} tokens from left."
                f"Original char_len={len(input_text)}.")
            input_ids_to_model = input_ids_full[:, -max_len_for_model_input:]
        
        if input_ids_to_model.size(1) == 0:
            print("ERROR (_next_token_entropy): input_ids became empty after truncation. Returning 0 entropy.")
            return torch.tensor(0.0, device=device)

        # 将处理后的 input_ids_to_model 移动到正确的设备
        input_ids_to_model = input_ids_to_model.to(device)

        outputs = model(input_ids_to_model)
    ...
    def _watermarking(self, ids, logits, secret_string, measure_threshold):
        ...
        if len(ids[0]) <= measure_threshold:
            ...
            current_generated_token_ids = ids[0] 
            measure_text_str = self.watermark_tokenizer.decode(current_generated_token_ids)
            
            
            measure_entroy = self._next_token_entropy(measure_text_str, self.measure_model, self.measure_tokenizer, self.device)
            if measure_entroy >= self.alpha:
                embedding = self.embedding_model.encode(measure_text_str, convert_to_tensor=True)
    ...
    def detection(self, text):
        ...
        for i in range(len(watermark_ids[0])):
            if i <= self.measure_threshold:
                ...
                measure_text_ids_segment = watermark_ids[0][:i] # 使用 watermark_ids
                measure_text_str = self.watermark_tokenizer.decode(measure_text_ids_segment) # 使用正确的片段解码
                
                
                measure_entroy = self._next_token_entropy(measure_text_str, self.measure_model, self.measure_tokenizer, self.device)
                if measure_entroy >= self.alpha:
                    e = self.embedding_model.encode(measure_text_str, convert_to_tensor=True, device=self.device)
                    te = self.transform_model(e).tolist()
                    te = [1.0 if x>0.0 else 0.0 for x in te]
                    ve = torch.tensor([te[i] for i in self.mapping_list], device=self.device)
                    s = ve[watermark_ids[0][i]]
        ...
        if not score: # 处理score列表为空的情况，例如文本太短，i从未大于measure_threshold且满足条件
            # print("DEBUG (detection): Score list is empty, returning 0.0 as default.")
            return 0.5 # 或者根据你的逻辑返回一个合适的值，例如0.5如果0.5是随机基线
